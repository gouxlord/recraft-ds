...
const DataTable = ({ 
  ...
  const getSortIcon = (columnKey) => {
    ...
    return sortConfig.direction === 'asc' 
      ? <CaretUp size={16} className="text-primary-600" weight="fill" />
      : <CaretDown size={16} className="text-primary-600" weight="fill" />;
  };
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div className="flex flex-wrap gap-2">
          ...
          <div className="relative">
            ...
            {showColumnSelector && (
              <div className="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-large z-10">
                <div className="p-2">
                  <p className="text-xs font-medium text-gray-700 mb-2">Affichage des colonnes</p>
                  {columns.map(column => (
                    <label key={column.key} className="flex items-center gap-2 p-1 hover:bg-gray-50 rounded">
                      <Checkbox
                        checked={!hiddenColumns.has(column.key)}
                        onCheckedChange={(checked) => !checked && toggleColumn(column.key)}
                      />
