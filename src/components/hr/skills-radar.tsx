...
export type SkillCategory = 'soft' | 'technical' | 'management';
...
const SkillsRadar = ({ 
  ...
  const defaultSkills: SkillData[] = skills.length > 0 ? skills : [
  ...
  const getCategoryColor = (category: SkillCategory): 'default' | 'secondary' | 'destructive' | 'outline' | 'success' | 'warning' => {
    const colors: Record<SkillCategory, 'default' | 'secondary' | 'destructive' | 'outline' | 'success' | 'warning'> = {
      soft: 'default',
      technical: 'secondary',
      management: 'success'
    };
  ...
  const getProgressVariant = (current: number, target: number): 'default' | 'success' | 'warning' => {
